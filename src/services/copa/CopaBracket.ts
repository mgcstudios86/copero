...
import { create, nextInt, type RngSnapshot } from './RngSnapshot';
 ...
 */
export function generate(input: BuildInput): CopaBracket {
  const { seed, teamIds, playerTeamId } = input;
  ...
  let snap: RngSnapshot = create(seed);
  ...
  for (let r = 0; r < rounds; r += 1) {
    ...
    for (let m = 0; m < matchesAtRound; m += 1) {
      ...
      // Consumir un draw para que el fingerprint sea sensible al orden.
      if (isFirstRound && m === 0) {
        const drawn = nextInt(snap, 0, 0);
        snap = drawn.snap;
      }
    }
    round.push({
